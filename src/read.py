...
def stratified_split_samples_pd(
    ...
    """
    use pandas to stratify samples by population (pop_col),
    and split them into reference set (ref) and test set (test),
    and write samples and their corresponding populations to output files.

    output file format (TSV):
    sample_id <tab> population

    return: (ref_df, test_df) two DataFrames, only contain sample_col and pop_col
    """
    # load sample table
    df = pd.read_csv(meta_path, sep=sep)

    # for reproducibility, shuffle the whole dataframe
    df = df.sample(frac=1.0, random_state=seed).reset_index(drop=True)

    ref_indices = []
    test_indices = []

    # stratify by population, and split into 75/25
    ...
    # write: two columns sample_id + population
